...
const INITIAL_MESSAGES: Message[] = [
  ...
  },
  {
    id: "m2",
    senderId: "1",
    text: "It is going great! Just built the dynamic theme engine.",
    timestamp: "10:01 AM",
    type: "text",
  },
];
...
export default function Home() {
  ...
  React.useEffect(() => {
    ...
    const isCompatible =
      mode === "post"
        ? currentPlatformConfig.supportsPosts
        : currentPlatformConfig.supportsChat;

    if (!isCompatible) {
      // Find the first compatible platform
      const firstCompatible = Object.values(PLATFORMS).find((p) =>
        mode === "post" ? p.supportsPosts : p.supportsChat
      );
      if (firstCompatible) {
  ...
  const [post, setPost] = React.useState<Post>({
    ...
    authorId: "1",
    content:
      "Just launched Konvo! Replicate any chat or post with ease. #buildinpublic #saas",
    timestamp: "2h",
    engagements: {
      likes: 124,
      comments: 12,
